...
class Interface:
    ...
    def generate(self, guard_type: str = 'none', indent_src: bool = True, tabsize: int|None=None) -> str:
        code = [self.generate_struct(),
                self.generate_func_helper()]
        ...
        code.insert(0, f'/* Code generated by interfacer.py at UTC {now} */\n')
...
def indent_source_code(src: str, tabsize:int|None = None):
    lines = src.split('\n')
    ...
    for i, line in enumerate(lines):
        lines[i] = indent(line, tab * level)
        l, r = line.count('{'), line.count('}')
        if r > 0:
            dedent(lines[i])
            lines[i] = indent(line, tab * (level - 1))
        level += l - r
