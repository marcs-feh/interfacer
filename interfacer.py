...
class Interface:
    ...
    def generate(self, guard_type: str = 'none', indent_src: bool = True, tabsize: int|None=None) -> str:
        ...
        src = add_header_guard(src, guard_type)
    ...
    def generate_file(self, outfile: str,guard: str = 'none', indent_src: bool = True, tabsize: int|None = None):
        src = self.generate(guard_type=guard, indent_src=indent_src, tabsize=tabsize)

        with open(outfile, 'w') as f:
            n = f.write(src)
            stderr.write(f'Wrote {n}B to {outfile}\n')

def add_header_guard(src: list[str], mode: str) -> list[str]:
